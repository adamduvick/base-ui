...
//!
//! Ported from `@base-ui/utils/error`.
//! Logs each unique message at most once via `web_sys::console::error_1`.
...
/// Log an error message prefixed with "Base UI: ", deduplicating by message content.
///
/// Only active in debug builds (`cfg(debug_assertions)`). In release builds this is a no-op.
///
/// Maps to `error(...messages)` in the React version.
pub fn error(messages: &[&str]) {
    if cfg!(debug_assertions) {
        let message_key: String = messages.join(" ");
        let mut set = SEEN.lock().unwrap();
        if !set.contains(&message_key) {
            set.insert(message_key.clone());
            web_sys::console::error_1(&format!("Base UI: {message_key}").into());
        }
    }
}
...
///
/// Maps to `reset()` in the React version.
pub fn reset() {
    if cfg!(debug_assertions) {
        let mut set = SEEN.lock().unwrap();
        set.clear();
...
mod tests {
    ...
    fn reset_clears_seen_messages() {
        // After reset, the set should be empty
        reset();
    ...
    fn deduplication_logic() {
        ...
        {
            let mut set = SEEN.lock().unwrap();
            assert!(!set.contains("test message"));
            set.insert("test message".to_string());
            assert!(set.contains("test message"));
